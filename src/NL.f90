...
!  Parameters:
!
!    Input, REAL(wp) ::  ARG, the upper limit of integration.
!
!    Output, REAL(wp) ::  CUM, CCUM, the Normal density CDF and
!    complementary CDF.
!
!  Local Parameters:
!
!    Local, REAL(wp) ::  EPS, the argument below which anorm(x)
...
!
  ...
  REAL(wp) , parameter, dimension ( 5 ) :: a = (/ &
  ...
  REAL(wp) , INTENT(OUT) :: ccum
  REAL(wp) , INTENT(OUT) :: cum
  ...
  REAL(wp) ::  eps
  integer :: i
  REAL(wp) , parameter, dimension ( 6 ) :: p = (/ &
  ...
  REAL(wp) , parameter, dimension ( 5 ) :: q = (/ &
    1.28426009614491121D+00, &
    4.68238212480865118D-01, &
    6.59881378689285515D-02, &
    3.78239633202758244D-03, &
    7.29751555083966205D-05 /)
  REAL(wp) , parameter :: root32 = 5.656854248D+00
  REAL(wp) , parameter :: sixten = 16.0D+00
  REAL(wp) ::  temp
  REAL(wp) , parameter :: sqrpi = 3.9894228040143267794D-01
  REAL(wp) , parameter :: thrsh = 0.66291D+00
  REAL(wp) ::  x
  REAL(wp) ::  xden
  REAL(wp) ::  xnum
  REAL(wp) ::  y
...
!    2, scaling is specified by the input DIAG vector.
!
!    Input, REAL(wp) ::  FACTOR, determines the initial step bound.
...
!    Input, REAL(wp) ::  QTB(N), the first N elements of the vector Q'*B.
!
!    Input, REAL(wp) ::  DELTA, an upper bound on the euclidean norm
...
!
!    Output, REAL(wp) ::  X(N), the least squares solution of the system
...
!
!    Input/output, REAL(wp) ::  V(N).  On input, V must contain the
...
!
!    Input, REAL(wp) ::  A(N), the vector to be printed.
!
